...
#data_dir <- here("data/input_node_files/")

clean_test_input <- function(data_dir, file_name){
  ...
  d_input <- 
    map_df(input_foldrs, function(folder){
      input_files <- here("data", "input_node_files_test/", folder, list.files(paste0(data_dir, "/", folder)))
